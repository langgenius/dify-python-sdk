...
class TestAsyncAPIParity(unittest.TestCase):
    ...
    def test_dify_client_api_parity(self):
        ...
        async_methods = {
            name for name in dir(AsyncDifyClient) if not name.startswith("_")
        }
        ...
        # Verify parity
        self.assertEqual(
            sync_methods, async_methods, "API parity mismatch for DifyClient"
        )
    ...
    def test_chat_client_api_parity(self):
        ...
        async_methods = {
            name for name in dir(AsyncChatClient) if not name.startswith("_")
        }
        ...
        self.assertEqual(
            sync_methods, async_methods, "API parity mismatch for ChatClient"
        )
    ...
    def test_completion_client_api_parity(self):
        ...
        sync_methods = {
            name for name in dir(CompletionClient) if not name.startswith("_")
        }
        async_methods = {
            name for name in dir(AsyncCompletionClient) if not name.startswith("_")
        }
    ...
    def test_workflow_client_api_parity(self):
        ...
        sync_methods = {
            name for name in dir(WorkflowClient) if not name.startswith("_")
        }
        async_methods = {
            name for name in dir(AsyncWorkflowClient) if not name.startswith("_")
        }
    ...
    def test_workspace_client_api_parity(self):
        ...
        sync_methods = {
            name for name in dir(WorkspaceClient) if not name.startswith("_")
        }
        async_methods = {
            name for name in dir(AsyncWorkspaceClient) if not name.startswith("_")
        }
    ...
    def test_knowledge_base_client_api_parity(self):
        ...
        sync_methods = {
            name for name in dir(KnowledgeBaseClient) if not name.startswith("_")
        }
        async_methods = {
...
class TestAsyncClientMocked(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_async_send_request(self, mock_httpx_async_client):
        ...
        async with AsyncDifyClient("test-key") as client:
            await client._send_request("GET", "/test")
