...
class TestKnowledgeBaseClient(unittest.TestCase):
    ...
    def _get_dataset_kb_client(self):
        return KnowledgeBaseClient(
            self.api_key, base_url=self.base_url, dataset_id=self.dataset_id
        )
...
class TestChatClient(unittest.TestCase):
    ...
    @patch("dify_client.client.httpx.Client")
    def test_create_chat_message_with_vision_model_by_remote_url(
        ...
        mock_response.json.return_value = {
            "answer": "I can see this is a test image description."
        }
        mock_response.status_code = 200
    ...
    @patch("dify_client.client.httpx.Client")
    def test_create_chat_message_with_vision_model_by_local_file(
        self, mock_httpx_client
    ):
        # Mock the HTTP response
        mock_response = Mock()
        ...
        mock_response.json.return_value = {
            "answer": "I can see this is a test uploaded image."
        }
        mock_response.status_code = 200
    ...
    def test_get_conversation_messages(self, mock_httpx_client):
        ...
        mock_response.json.return_value = {
            "answer": "Here are the conversation messages."
        }
        mock_response.status_code = 200
        ...
        # Create client with mocked httpx
        chat_client = ChatClient(self.api_key)
        response = chat_client.get_conversation_messages(
            "test_user", "test-conversation-id"
        )
        self.assertIn("answer", response.text)
...
class TestCompletionClient(unittest.TestCase):
    ...
    def test_create_completion_message(self, mock_httpx_client):
        ...
        mock_response = Mock()
        mock_response.text = (
            '{"answer": "The weather today is sunny with a temperature of 75°F."}'
        )
        mock_response.json.return_value = {
            "answer": "The weather today is sunny with a temperature of 75°F."
        }
    ...
    @patch("dify_client.client.httpx.Client")
    def test_create_completion_message_with_vision_model_by_remote_url(
        self, mock_httpx_client
    ):
        # Mock the HTTP response
        mock_response = Mock()
        mock_response.text = (
            '{"answer": "This is a test image description from completion API."}'
        )
        mock_response.json.return_value = {
            "answer": "This is a test image description from completion API."
        }
    ...
    @patch("dify_client.client.httpx.Client")
    def test_create_completion_message_with_vision_model_by_local_file(
        self, mock_httpx_client
    ):
        # Mock the HTTP response
        mock_response = Mock()
        ...
        mock_response.json.return_value = {
            "answer": "This is a test uploaded image description from completion API."
        }
        mock_response.status_code = 200
...
class TestDifyClient(unittest.TestCase):
    ...
    def test_get_application_parameters(self, mock_httpx_client):
        ...
        mock_response = Mock()
        mock_response.text = (
            '{"user_input_form": [{"field": "text", "label": "Input"}]}'
        )
        mock_response.json.return_value = {
            "user_input_form": [{"field": "text", "label": "Input"}]
        }
